...
/**
 * Produce ThreadHandlers that create and control execution of threads for
 * iterating over List<T> objects
 */
public class ThreadHandlerFactory {
	...
	/**
	 * Given some context, produce a ThreadHandler for iterating over the list of
	 * elements provided in the context
	 *
	 * @param <T>
	 * @param context
	 * @return
	 ...
	 * @return
	 */
	protected <T> Thread[] getThreads(ThreadHandler<T> thandler, List<T> list, ElementHandler<T> ehandler,
			int[] lengths) {
	...
	/**
	 * Get the number of threads to use. The number returned here will never be
	 * greater than max. It may be less than min, but only in the case where
	 * elements is also less than min.
	 *
	 * @param max
	 * @param min
	 * @param elements
	...
	protected int getThreadCount(int max, int min, int elements, int divisor) {
		...
		// Divisor allows clients to scale threads in proportion to the number of
